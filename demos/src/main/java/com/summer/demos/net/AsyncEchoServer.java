...
//
///**
// * AsyncCounterServer is built on top Java nio2 asynchronous socket channels.
// * It can receive messages from clients and echo the response back.
// *
// * The responsibility for managing server instances is with {@link ServerManager}. That class should be
// * used for starting and stopping server instances.
// * <P>
// * Every Server has an server name and should be configured to run on free port.
// * <P>
// * To make remote requests to this class refer to {@link com.alex.asyncexamples.client.AsyncEchoClient}
// * <P>
// *
// * <dev-notes>
...
// * <LI> Non-direct byte buffers which don't.
// * Recommendation from JDoc is to use indirect byte buffers.
...
// * See: http://docs.oracle.com/javase/7/docs/api/java/nio/ByteBuffer.html
// * </UL>
// * <LI> Threads
// * <UL>
// * <LI> Instantiation of AsyncCounterServer instance can happen on any thread - the thread running a test, the main execution
// * test of the program.
// * <LI> The ServerManager has a thread group which names threads as: ServerManager_Cache_TP_Thread. AsyncCounterServer instances
// * will set up a Completion Handler callback to receive channel events on this thread.
// * <LI> A cached thread pool is used for the channel group (this is a collection of all channels for a server instance).
// * Thread from this pool will have the name: %Server_name%_Group_Thread. Invocations to the command handler completed() methods
// * will happen on threads from this pool. In addition, subsequent CompletionHandler callbacks are set up on this thread.
// * <LI> Everytime the server accepts a client request, the counter <code>acceptCounter</code> is incremented. Everytime a request
// * finished this counter is decrement.  Any request to shutdown this server will check this counter and will try to wait until
// * it is zero (signifying there are no requests) and then close down. It won't wait forever.  So it is possible to have some corruption
// * but just very unlikely.
// * <LI> A request will not be processed if the Server has been shutdown.
// * </UL>
// * <LI>Risks
// * There some documented risks associated with using nio2 async channels.
// * <UL>
// * <LI> Long lived operations on the CompleteHandler Callbacks could starve a fixed sized thread pool.</LI>
// * Several mitigation strategies for this. the one I used was to use a CachedThreadPool which would increase in pool size as needs be.
// * There are also read / write timeout set in the callback logic.  To make this server even more sophisticated logic could be added
// * so that if a counter has been changed and then the write back to the client fails, the change rolls back.
// * <LI> Too many reads happening at the same time and overwhelming memory</LI>
// * A mitigation strategy here would be to throttle using a pool of ByteBuffers.  I didn't implement this in this
// * in this example as according to Alan Bateman's JavaOne presentation it was possible to reach 10,000 connection
// * without this happening.
// * <LI>
// * </UL>
// * See: http://openjdk.java.net/projects/nio/presentations/TS-4222.pdf for more details.
// *
// * @author Alex
// */
...
//				Executors.newCachedThreadPool( r -> new Thread(r, "Group_Thread_" + ( threadcnt ++ ))));
//    }
//
//
...
//		AsyncEchoServer echoserver = new AsyncEchoServer("AsyncEchoServer");
//		echoserver.open(new InetSocketAddress("127.0.0.1", 8099));
//		echoserver.run();
